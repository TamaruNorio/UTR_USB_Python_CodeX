...
| `UHF_BUZZER_pi` | ブザー制御、単音側 | あり | ブザー制御 | ACK/NACK、ブザー動作 | 実機に音が出るため手順確認後のみ |
| `UHF_BUZZER_pipipi` | ブザー制御、連続音側 | あり | ブザー制御 | ACK/NACK、ブザー動作 | 実機に音が出るため手順確認後のみ |
...
- 送信出力を書き換える処理は、実機状態へ影響する可能性があるため、手順と接続条件の確認なしに行わないこと。
