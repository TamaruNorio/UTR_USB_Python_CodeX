...
本リポジトリでは、仕様書・取扱説明書・実機ログで確認できた範囲だけを実装対象にします。根拠が不足している動作は、コード上では実装済みとして扱わず、READMEやdocsでは「未対応」「要実機確認」と明記します。

現在の重要な制約は以下です。

- USB接続は仮想COMポートとして扱います。
- 通信速度の標準確認値は 115200 bps です。
- 現在のアンテナ順次Inventoryは、実機確認済みの UTR-SUN02-4CH を主対象にします。
- UTR-SUN02V-8CH / UTR-SUN02-8CH は、ROMシリーズ名と接続確認対象の識別までは扱いますが、4CH用のアンテナ切替設定書き込みを流用しません。
...
- Ctrl+Cなどの中断時も、可能な限り受信残データを読み捨ててから元のコマンドモード用アンテナ設定へ復元します。
...
- `inventory_results.txt`、`inventory_results.csv`、`inventory_results.json`、`inventory_results_legacy_*.csv` はGit管理に入れません。
...
Inventory結果ファイルに含まれる実測PC/UII、PC/EPC、ログも、公開時は必ずマスクしてください。
